...
export default function Home() {
  ...
  function handleViewportReady(bounds) {
    ...
    // Dynamic padding scales with viewport size — prevents markers from appearing/disappearing at edges.
    // At zoom 3 (whole country) padding is large; at zoom 16 it's tiny but sufficient.
    const latRange = bounds.getNorth() - bounds.getSouth();
    const lngRange = bounds.getEast() - bounds.getWest();
    const PADDING_LAT = latRange * 0.1;
    const PADDING_LNG = lngRange * 0.1;
    const paddedWest = bounds.getWest() - PADDING_LNG;
    const paddedSouth = bounds.getSouth() - PADDING_LAT;
    const paddedEast = bounds.getEast() + PADDING_LNG;
        ...
        .map((prop, index) => {
          ...
          return {
            ...
            "properties": {
              ...
              "home_id": prop._id,
              "price": prop.price ?? 0,
              "display_price": formatPrice(prop.price ?? 0)
            },
            "id": prop._id,
            "geometry": {
              "type": "Point",
